...
Subclass `DefaultState`, `DefaultFinalState`, or
their [data](https://kstatemachine.github.io/kstatemachine/pages/transitions/typesafe_transitions.html) analogs
`DefaultDataState`, `DefaultFinalDataState`. Then you can easily distinguish your states by type when observing state
changes:
...
`onFinished {}` fires when the state finishes — i.e. when its child `FinalState` is entered (exclusive mode) or all
parallel children have finished (parallel mode). This is equivalent to listening for the generated `FinishedEvent`.
...
val listener = onActiveAllOf(State1, State2, State3) { allActive ->
    println("all three active: $allActive")
}
```

Both functions accept an optional `notifyOnSubscribe: Boolean` parameter (default `false`). When set to `true`,
the callback is invoked immediately at subscription time with the current status.

Both functions return a `GroupListener` whose `unsubscribe()` method removes all internal listeners at once.
Call it when you no longer need the subscription to avoid memory leaks:

```kotlin
val listener = onActiveAllOf(State1, State2, notifyOnSubscribe = true) { /* ... */ }
// later:
...
After machine setup you can look up states by name or type:

```kotlin
// by name (returns null if not found)
val s = machine.findState("Yellow")
// by name (throws if not found)
val s = machine.requireState("Yellow")

// by type — throws if more than one match
val s = machine.findState<YellowState>()
val s = machine.requireState<YellowState>()
```

Both families accept `recursive: Boolean = true` to control whether to search child states.
Note: search does not descend into nested `StateMachine` instances.

## Querying active states

```kotlin
val active: Set<IState> = machine.activeStates()
// pass selfIncluding = true to include the machine itself in the result
val activeWithMachine = machine.activeStates(selfIncluding = true)
```
...
concept,
see [typesafe transitions](https://kstatemachine.github.io/kstatemachine/pages/transitions/typesafe_transitions.html)
section.
...
See [Typesafe transitions](https://kstatemachine.github.io/kstatemachine/pages/transitions/typesafe_transitions.html)
for the `DataState` + `DataEvent` API.
...
2. Generation and processing of special `FinishedEvent`. This option is also available for composite states and useful
   ...
   `FinishedEvent` carries two fields:
    - `state: IState` — the state whose finalization generated this event
    - `data: Any?` — non-null only when the finished state is a `FinalDataState`; contains its data value

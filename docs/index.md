...
top-level `FinalState`. In `ChildMode.PARALLEL` state machine finishes when all its children has finished. On finish,
state machine notifies its listeners with `onFinished()` callback.
